...
Helm does not upgrade CRDs in `crds/` (see [helm/README.md](../helm/README.md)), so the order matters. Apply the CRDs **before** `helm upgrade`, otherwise the chart removes the webhook Service while the old CRDs still reference it for conversion.
